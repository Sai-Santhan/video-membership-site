...
from app.playlists.schemas import PlaylistVideoAddSchema
...
@login_required
def playlist_create_post_view(request: Request,
                              title: str = Form()):
    raw_data = {
        "title": title,
...
@login_required
def playlist_video_add_view(request: Request,
                            db_id: uuid.UUID,
...
def playlist_remove_video_item_view(request: Request,
                                    db_id: uuid.UUID,
                                    is_htmx=Depends(is_htmx),
                                    index: Union[int, None] = Form(default=None)):
    if not is_htmx:
    ...
    try:
        ...
        return HTMLResponse("Error Occured. Please reload the page.")
